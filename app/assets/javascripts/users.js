$(document).ready(function() {
	// $('div#main-container').effect("highlight", {}, 3000);
	// $("h1").effect("highlight", {}, 3000);
	...
	// $("p.checkin").each do	
	// var p_checkin_id = $("p.checkin").attr('id');
	// var intervalID = window.setInterval(function() { 
	//     $.getJSON("/users/" + p_checkin_id + "/checkin_label.json",
	//       function(data) {
	//         console.debug ('request')
	//         if (data.treated == 1) {
	//           $("#" + p_checkin_id).html("<p class='checkin' id='" + data.user_id + "'>" + data.checkin_label + "</p>");
	//           clearInterval(intervalID);
	//           console.debug ('interval end')
	//         }
	//       })
