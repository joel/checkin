class NotifierMailer < ActionMailer::Base
  ...
  def checkin_notification(user_id, token_type, motivation)
    ...
    mail(:to=>"#{@user.name} <#{@user.user.email}>", :subject=>"I'm at #{Settings.app.name} to #{token_type} for #{motivation}")
  end
  
  def notify_followers(follower_id,user_id, token_type, motivation)
    @follower = User.find_by_id(follower_id.to_i)
    @user = User.find_by_id(user_id.to_i)
    mail(:to=>"#{@follower.name} <#{@follower.user.email}>", :subject=>"#{@user.name} is at #{Settings.app.name} to #{token_type} for #{motivation}")
  end
  ...
  def notify_people(sender_id, notification_id, member_id)
    ...
    mail(:to=>"#{@member.name} <#{@member.user.email}>",:subject=>"#{@sender.firstname} warn you about #{Settings.app.name}")
  end
